...
#### [R] `share_token` removed from the public API contract
- `ResultResponse.ShareToken`/`json:"share_token"` removed (`result_query.go`). `TestResultRepository.FindByShareToken` removed from the interface and its Postgres implementation — was dead code (zero callers besides its own mock). Confirmed via `mockery` regeneration touching only `mock_TestResultRepository.go`.
- Entity field `TestResult.ShareToken` and DB column `test_results.share_token` are unchanged — dropping the column needs a migration and isn't required for MVP; the value is still generated at submit time, just never returned by any endpoint. See `TECHNICAL_DOCUMENTATION.md` Section 5.3 for the "why" and the explicit "don't resurrect this without a new design discussion" note.

#### [C] Swagger annotations — CSRF documentation gap closed, full endpoint coverage verified
- The 4 CSRF-protected endpoints (`POST /v1/assessment/submit`, `PATCH /v1/account/profile`, `POST /v1/account/delete-request`, `POST /v1/account/delete-request/cancel`) previously had zero mention of the `X-CSRF-Token` requirement anywhere in their Swagger docs — Swagger has no native concept of cookie-priming, so this is now spelled out in each endpoint's `@Description` plus an explicit `@Param X-CSRF-Token` and `@Failure 403`. Cloudflare Turnstile's `cf_turnstile_response` contract (register/login/forgot-password) was already fully documented — audited, not a gap.
- Verified 1:1: every route registered in `router.go` (26 total) has a matching `@Router` Swagger annotation — no missing endpoint, no orphaned doc block for a route that no longer exists.
- `docs/swagger.json`/`swagger.yaml`/`docs.go` themselves are NOT regenerated by this change — `make swag` still needs to run (standing convention, not run here). README gains an "API Contract" section: where the file lives, how to regenerate, and the `/v2`-for-breaking-changes rule now in effect since this is the declared BE-MVP-feature-complete gate.

#### Scope note
- Manual end-to-end smoke test (Guest + Member happy path via Swagger UI, per the ticket's own checklist) was NOT run from this environment — no live dev server available to drive. The checklist is written out in `ticket-27-fe-handoff-contract.md` for the user/QA to execute before merge.

## [UNRELEASED] — 2026-07-17 (3)

### Production deployment artifacts & trusted-proxy IP extraction (TICKET-26)

#### [F] `c.RealIP()` no longer blindly trusts a client-supplied `X-Forwarded-For` header (security fix, live in every environment)
- Echo's default behavior when `IPExtractor` is unset trusts `X-Forwarded-For` unconditionally, regardless of whether a real reverse proxy is in front — meaning any client could already spoof their rate-limit bucket (TICKET-22) and the `remoteip` sent to Cloudflare Turnstile (TICKET-11) just by setting a header. New `http.ParseTrustedProxies` (env `TRUSTED_PROXIES`, comma-separated CIDRs/IPs) is now always set explicitly on the Echo instance — `echo.ExtractIPDirect()` (raw TCP peer, unspoofable) when empty, `echo.ExtractIPFromXFFHeader` scoped to the given trusted ranges otherwise. Found while implementing TICKET-26's trusted-proxy task; more severe than the ticket originally framed it (not just a prod-deployment correctness issue).

#### [A] Fail-fast production config validation (`internal/config.RequireProduction`)
- `cmd/api` and `cmd/worker` now refuse to boot when `APP_ENV=production` and any critical field is empty or still at its insecure dev default: `JWT_SECRET`, `DB_PASSWORD`, `S3_ACCESS_KEY`/`S3_SECRET_KEY` (api+worker); `GEMINI_API_KEY`/`GEMINI_MODEL`/`TURNSTILE_SECRET_KEY`/`ALLOWED_ORIGINS` (api only); `SMTP_USER`/`SMTP_PASSWORD` (worker only). Deliberately excludes `REDIS_PASSWORD` — no insecure code-level default exists for it, and unauthenticated Redis on a private network is a legitimate deployment choice, not a mistake to fail the boot over.

#### [A] Production Docker Compose, Caddy reverse proxy, deploy runbook
- `docker/docker-compose.prod.yml` — standalone file (not a dev overlay, to avoid Compose's fragile multi-file list-merge semantics), no MinIO/Mailpit, Postgres/Redis not published to the VM's public interface (Postgres stays on `127.0.0.1` so the host-run `scripts/backup.sh` from TICKET-13 keeps working), `restart: unless-stopped` everywhere.
- `docker/Caddyfile` — 5-line automatic-TLS reverse proxy for `api.yourpersonas.com`.
- `docker/Dockerfile` gains a `seed` binary build/copy step — was missing entirely despite the ticket's own deploy sequence naming it ("compose up → migrate → seed → smoke test"); found while writing the runbook.
- `docs/deploy_runbook.md` — deploy-from-zero, cron registration, trusted-proxy verification, graceful-shutdown verification, redeploy, rollback. External/manual prerequisites (DNS, R2 bucket + Lifecycle rule, Brevo, Turnstile site key) called out explicitly as things this runbook can guide but not execute.
- **Scope honesty**: acceptance criteria requiring a real VM/DNS/R2/Brevo (live `https://api.yourpersonas.com` smoke test, R2 Lifecycle rule screenshot, cron running against production R2) are marked as pending manual execution in `ticket-26-production-deployment.md`, not claimed done — same treatment TICKET-13's backup script got. `cmd/api` needs `make wire` re-run (2 new constructor params); all other packages build/vet/test clean.
