...
Two packages (`internal/application/assessment`, `internal/application/pdf`) generate their mocks `inpackage: true` into a `mock_*_test.go` file instead of a `pkgname/mocks` subpackage — their interfaces reference a type defined in that same package (`assessment.SubmitResponse`, `pdf.PDFData`), and a subpackage mock importing back into the package under test hits Go's "import cycle not allowed in test" the moment that package's own internal `_test.go` files import it. `inpackage` mode sidesteps this entirely (no cross-package import, and the file is test-only — never compiled into the production binary). Every other package's mocks live in a plain `pkgname/mocks` subpackage.
